<%@ Page MasterPageFile="~/template.master" Inherits="TFU03_SFLUSRS" CodeFile="TFU03_SFLUSRS.aspx.cs" Language="C#" %>

<%@ Register TagPrefix="gx" Namespace="com.sabratec.dotnet.framework.web.controls" Assembly="GXDotnet" %>

<asp:Content ID="data" ContentPlaceHolderID="GXPagePlaceHolder" runat="server">
    <div id="gx_screenArea">
        <div class="command-toolbar" id="hostKeysToolbar" runat="server"></div>
        <action-bar mode="multiple">
            <input id="actionBar" runat="server" />
        </action-bar>
        <div class="main-content" id="mainContent">
            <div class="headline-container">
                <h1 class="headline" id="Headline" runat="server">USERS LIST WITH PRIVTE DEFULTS</h1>
            </div>

            <div class="centered-flex related-fields-area wrapper-container">
                <gal-input label-text="Option:" label-size="auto" input-size="large">
                    <input id="option_OPTION" runat="server" />
                    <input id="optionDesciption_OPTIOND" runat="server" />
                </gal-input>
                <gal-input label-text="Program name:" label-size="auto" input-size="small" no-description>
                    <input id="programName_PGM_NAME" class="no-color" runat="server" />
                </gal-input>
                <gal-input label-text="System:" label-size="auto" input-size="large">
                    <input id="system_SYS" class="no-color" runat="server" />
                    <input id="systemDescription_SYSD" class="no-color" runat="server" />
                </gal-input>
            </div>
            <div class="table-container">
                <table id="usersDefaultsList_SFLUSRS" runat="server" class="table gal6-table-striped">
                    <thead>
                        <tr class="grid-header">
                            <th class="option-column">
                                <input type="checkbox" id="masterCheckBox" />
                            </th>
                            <th>
                                <gal-input label-size="" input-size="small" no-description>
                                    <input id="userFilter_USER" runat="server" />
                                </gal-input>
                                <span>User name</span>
                            </th>
                            <th>
                                <gal-input label-size="" input-size="large" no-description>
                                    <input id="userNameFilter_USER_NME" runat="server" />
                                </gal-input>
                                <span>User description</span>
                            </th>
                        </tr>
                    </thead>

                    <tbody>
                        <tr>
                            <td class="option-column">
                                <input class="row-selector-cell" id="opt_OPT_USR" runat="server" />
                                <input type="checkbox" galtype="checkbox" />
                            </td>
                            <td><span id="user_USER" runat="server"></span></td>
                            <td>
                                <span id="userName_USER_NME" runat="server"></span>
                                <action-bar mode="single"></action-bar>
                            </td>
                        </tr>
                    </tbody>
                </table>
                <grid-scroller></grid-scroller>
            </div>
            <input id="pageIndex" runat="server" style="display: none;" />
            <div class="message-container">
                <span class="invalid-feedback" id="msg_MSG1" runat="server"></span>
            </div>
        </div>
    </div>
</asp:Content>
